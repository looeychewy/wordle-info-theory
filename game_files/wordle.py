...
def find_match(play_guess: str, final_answer: str) -> str:
    ...
    matches = [""] * len(final_answer) # Output list to hold colorized letters
    ...
    for idx, (guess_letter, answer_letter) in enumerate(zip(play_guess, final_answer)):
        if guess_letter == answer_letter:
            ...
            answer_letters[idx] = None # Set to None instead of using .pop() to preserve list length
    ...
    for idx, guess_letter in enumerate(play_guess):
        ...
        if guess_letter in answer_letters: # Yellow letter condition, letter in both words but not at same place
        ...
        else: # Gray letter condition, letter not in answer word at all
